...
public:
    ...
    //=============CONSTRUCTORS=============//
    constexpr vector_base() noexcept(noexcept(Allocator()))
        : allocator_   (Allocator()),
          start_value  (nullptr),
          end_value    (nullptr),
          end_capacity (nullptr) {
    }
    constexpr explicit vector_base(const Allocator& allocator) noexcept
        : allocator_   (allocator),
          start_value  (nullptr),
          end_value    (nullptr),
          end_capacity (nullptr) {
    }
    constexpr vector_base(size_type count, const_reference value, const Allocator& allocator = Allocator())
        : allocator_   (allocator),
    ...
    constexpr explicit vector_base(size_type count, const Allocator& allocator = Allocator())
        : allocator_   (allocator),
          start_value  (allocator_.allocate(count)),
          end_value    (start_value + count),
          end_capacity (end_value) {
    }
    constexpr vector_base(input_iterator<T> first, input_iterator<T> last, const Allocator& allocator = Allocator())
        : allocator_   (allocator),
    ...
    constexpr vector_base(const vector_base& another)
    ...
    constexpr vector_base(const vector_base& another, const Allocator& allocator)
        : allocator_   (allocator),
          start_value  (allocator_.allocate(another.capacity())),
          end_value    (start_value + another.size()),
          end_capacity (start_value + another.capacity()) {
    ...
    constexpr vector_base(vector_base&& another) noexcept
    ...
    constexpr vector_base(vector_base&& another, const Allocator& allocator) 
        : allocator_   (allocator),
          start_value  (nullptr),
          end_value    (nullptr),
          end_capacity (nullptr) {
    ...
    constexpr vector_base(std::initializer_list<T> init_list, const Allocator& allocator = Allocator())
        : allocator_   (allocator),
    ...
    //=============OPERATORS=============//
    constexpr vector_base& operator=(const vector_base& another) {
        allocator_   = another.allocator_;
        start_value  = allocator_.allocate(another.capacity());
        end_value    = start_value + another.size();
        end_capacity = start_value + another.capacity();
    ...
    constexpr vector_base& operator=(vector_base&& another) noexcept {
        if (this != &another) {
            allocator_   = std::move(another.allocator_);
            start_value  = another.start_value;
            end_value    = another.end_value;
    ...
    constexpr vector_base& operator=(std::initializer_list<T> init_list) {
    ...
    constexpr void shrink_to_fit() {
        /*
        https://en.cppreference.com/w/cpp/container/vector_base/shrink_to_fit
...
public:
    ...
    constexpr void swap(vector_base& another) noexcept {
...
template<typename T>
class vector : public m_vector::vector_base<T> {
public:
...
private:
    ...
    constexpr void tryRealloc() {
        if (this->size() >= this->capacity() - 1) {
            this->reserve(RESIZE_COEFFICIENT * this->capacity());
        }
    }

    constexpr void doRealloc(m_vector::vector_base<T>::size_type new_capacity) {
        typename m_vector::vector_base<T>::vector_iterator temp = typename m_vector::vector_base<T>::vector_iterator(this->allocator_.allocate(new_capacity));
        copy(this->begin(), this->end(), temp, new_capacity);

        this->allocator_.deallocate(this->start_value.getPtr(), this->capacity());

        this->end_value    = temp + std::min(this->size(), new_capacity);
        this->start_value  = temp;
        this->end_capacity = temp + new_capacity;
    }

public:
    constexpr typename m_vector::vector_base<T>::vector_iterator erase(m_vector::vector_base<T>::const_iterator pos) {
        copy(pos + 1, this->end(), pos);
        --(this->end_value);
        return pos;
    }
    constexpr typename m_vector::vector_base<T>::vector_iterator erase(m_vector::vector_base<T>::const_iterator first, m_vector::vector_base<T>::const_iterator last) {
        copy(last, this->end(), first);
        this->end_value = this->end_value - (last - first);
        return this->end_value;
    }

    constexpr void resize(m_vector::vector_base<T>::size_type count) {
        this->doRealloc(count);
    }
    constexpr void resize(m_vector::vector_base<T>::size_type count, m_vector::vector_base<T>::const_reference value) {
        if (this->size() == count) return;

        this->doRealloc(count);

        if (this->size() < count) {
            this->fill(this->end(), this->begin() + this->capacity(), value);
            this->end_value = this->end_capacity;
        }
    }

    constexpr void assign(m_vector::vector_base<T>::size_type count, m_vector::vector_base<T>::const_reference value) {
        resize(count);
        for (m_vector::vector_base<T>::size_type i = 0; i < count; ++i) {
            this->at(i) = value;
        }
    }
    constexpr void assign(input_iterator<T> first, input_iterator<T> last) {
        resize(last - first);
        copy(first, last, this->start_value);
    }
    constexpr void assign(std::initializer_list<T> init_list) {
        resize(init_list.size());
        copy(init_list.begin(), init_list.end(), this->start_value);
    }
};
};